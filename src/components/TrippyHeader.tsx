...
function TrippyHeader() {
  ...
  const scale = useTransform(scrollYProgress, [0.65, 0.8, 1], [1, 0.9, 1.25])
  const logoScale = useTransform(scrollYProgress, [0.8, 1], [0, 2])

  const rotate = useTransform(scrollYProgress, [0.25, 1], ['0deg', '90deg'])
...
const NUMBER_OF_SECTIONS = 25
const PADDING = `${100 / NUMBER_OF_SECTIONS / 2}vmin`

function generateSections(
  count: number, color: string, rotate: MotionValue, scale: MotionValue) {
  ...
  return (
    <Section background={color} scale={scale} rotate={rotate}>
...
const Trippy = ({
  ...
  return (
    <motion.div style={{ top }} className="absolute inset-0 overflow-hidden bg-black">
      {generateSections(0, 'white', rotate, scale)}
    </motion.div>
  )
}
